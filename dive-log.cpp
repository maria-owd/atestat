...
void parseLine(char *line, Dive &log) {
    ...
    if (sep) strcpy(log.date, urmatorulSeparator(sep));
    else strcpy(log.date, "N/A");

    if (sep) strcpy(log.time, urmatorulSeparator(sep));
    else strcpy(log.time, "N/A");

    if (sep) log.duration = atoi(urmatorulSeparator(sep));
    else log.duration = 0;

    if (sep) log.sac = atof(urmatorulSeparator(sep));
    else log.sac = 0.0f;

    if (sep) log.maxDepth = atoi(urmatorulSeparator(sep));
    else log.maxDepth = 0;

    if (sep) log.avgDepth = atoi(urmatorulSeparator(sep));
    else log.avgDepth = 0;

    if (sep) strcpy(log.mode, urmatorulSeparator(sep));
    else strcpy(log.mode, "N/A");

    if (sep) log.airTemp = atoi(urmatorulSeparator(sep));
    else log.airTemp = 0;

    if (sep) log.waterTemp = atoi(urmatorulSeparator(sep));
    else log.waterTemp = 0;

    if (sep) log.cylinderSize = atoi(urmatorulSeparator(sep));
    else log.cylinderSize = 0;

    if (sep) log.startPressure = atoi(urmatorulSeparator(sep));
    else log.startPressure = 0;

    if (sep) log.endPressure = atoi(urmatorulSeparator(sep));
    else log.endPressure = 0;

    if (sep) log.o2 = atoi(urmatorulSeparator(sep));
    else log.o2 = 0;

    if (sep) log.he = atoi(urmatorulSeparator(sep));
    else log.he = 0;

    if (sep) strcpy(log.location, urmatorulSeparator(sep));
    else strcpy(log.location, "N/A");

    if (sep) strcpy(log.gps, urmatorulSeparator(sep));
    else strcpy(log.gps, "N/A");

    if (sep) strcpy(log.diveMaster, urmatorulSeparator(sep));
    else strcpy(log.diveMaster, "N/A");

    if (sep) strcpy(log.buddy, urmatorulSeparator(sep));
    else strcpy(log.buddy, "N/A");

    if (sep) strcpy(log.suit, urmatorulSeparator(sep));
    else strcpy(log.suit, "N/A");

    if (sep) log.rating = atoi(urmatorulSeparator(sep));
    else log.rating = 0;

    if (sep) strcpy(log.visibility, urmatorulSeparator(sep));
    else strcpy(log.visibility, "N/A");

    if (sep) strcpy(log.notes, urmatorulSeparator(sep));
    else strcpy(log.notes, "N/A");

    if (sep) log.weight = atof(urmatorulSeparator(sep));
    else log.weight = 0.0f;

    if (sep) strcpy(log.tags, urmatorulSeparator(sep));
    else strcpy(log.tags, "N/A");
}
