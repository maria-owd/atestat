...
void afisareLinieIncheiere() {
    cout << "└"; linie(6); cout << "┴"; linie(21); cout << "┴"; linie(8); cout << "┴"; linie(10); 
    cout << "┴"; linie(14); cout << "┴"; linie(32); cout << "┘" << endl;
}
...
void editareScufundare() {
    ...
    if (index == -1) {
        cout << "Scufundarea cu numarul #" << diveNumber << " nu a fost gasita!" << endl;
        return;
    }

    Dive& dive = diveLogs[index];
    cout << "Editati detaliile pentru scufundarea #" << dive.diveNumber << ":" << endl;
    ...
    cout << "Editarea scufundarii #" << dive.diveNumber << " a fost finalizata!" << endl;
}
...
void stergereScufundare() {
    ...
    // Șterge scufundarea prin mutarea tuturor elementelor următoare cu o
    // poziție înapoi
...
void salvareJurnal() {
    ...
    for (int i = 0; i < numDives; ++i) {
        ...
        outFile << dive.diveNumber << ","
                << "\"" << dive.date << "\","
                << "\"" << dive.time << "\"," << dive.duration << "," << dive.sac << "," << dive.maxDepth << ","
                << dive.avgDepth << ","
                << "\"" << dive.mode << "\"," << dive.airTemp << "," << dive.waterTemp << "," << dive.cylinderSize
                << "," << dive.startPressure << "," << dive.endPressure << "," << dive.o2 << "," << dive.he << ","
                << "\"" << dive.location << "\","
                << "\"" << dive.gps << "\","
                << "\"" << dive.diveMaster << "\","
                << "\"" << dive.buddy << "\","
...
void top3Eficiente() {
    ...
    if (validDives == 0) {
        cout << "Nu exista scufundari cu SAC valid pentru a calcula statistici!" << endl;
        return;
    }
...
void durataMedie() {
    ...
    cout << "Durata medie a scufundarilor: " << fixed << setprecision(2) << medie << " minute" << endl;
}
...
void adancimeMedie() {
    ...
    cout << "Adancimea medie a scufundarilor: " << fixed << setprecision(2) << medie << " metri" << endl;
}
...
void locuriPopulare() {
    ...
    for (int i = 0; i < uniqueLocations; ++i) {
        if (locationCounts[i].count > 2) {  //
            cout << "│ " << setw(30) << left << locationCounts[i].location << " │ " << setw(14) << right
                 << locationCounts[i].count << " │" << endl;
        }
    }
...
/**
 * Executa optiunea de meniu selectata
 *
 * @param opt: optiunea selectata
 */
